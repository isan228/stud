// Тестовый скрипт для проверки создания платежа через Finik API с использованием библиотеки @mancho.devs/authorizer
require('dotenv').config();
const { createPaymentWithLibrary } = require('./utils/finik_with_library');

async function testFinikPaymentWithLibrary() {
  console.log('=== Тест создания платежа через Finik API (с библиотекой @mancho.devs/authorizer) ===\n');
  ...
  try {
    console.log('Отправка запроса к Finik API с использованием библиотеки @mancho.devs/authorizer...\n');
    const result = await createPaymentWithLibrary(testPaymentData);
...
// Запускаем тест
testFinikPaymentWithLibrary().catch(error => {
